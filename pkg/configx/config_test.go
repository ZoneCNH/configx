...
func TestConfigValidateRejectsNegativeTimeout(t *testing.T) {
	err := Config{Name: "configx", Timeout: -time.Second}.Validate()
	if err == nil {
...
func TestConfigSanitizeMasksSecret(t *testing.T) {
	sanitized := Config{Name: "configx", Timeout: time.Second, Secret: "plain-text"}.Sanitize()
	if sanitized.Secret != "***" {
		t.Fatalf("expected masked secret, got %q", sanitized.Secret)
	}
	if sanitized.Name != "configx" {
