...
- `examples/` 放可编译示例。
- `testkit/` 放测试辅助包。
- `scripts/` 放模板、边界、契约、安全、发布证据等检查脚本。
- `docs/` 放目标、设计、ADR、API、测试、发布和供应链文档。

测试文件与被测代码相邻，命名为 `*_test.go`。不要提交本地 `.omx/` runtime state、临时目录或生成的发布证据，除非 release 流程明确要求。
...
使用 `gofmt` 保持惯用 Go 格式。Package name 应短小、全小写并具备描述性，例如 `configx`、`validation` 或 `testkit`。公共 API 的 exported identifier 需要简洁注释。优先使用显式构造函数，避免全局可变状态。
...
- 覆盖 secret redaction，确保 raw secret 不泄漏。
- 覆盖显式 source loading，不隐式发现 `.env`、`production.yaml` 或 `/home/k8s/secrets/env/*`。
- 覆盖 validation、error kind、context cancellation/not found 的映射。
- 覆盖 source trace、contract schema 与 rendered template integration。

测试不得依赖机器特定路径、生产 secret 或隐式配置发现。

## 提交与 Pull Request 指南

提交信息遵循 Lore Commit Protocol：第一行说明为什么改，正文用 trailers 记录约束、验证和风险。实质性变更至少包含 `Tested:` 和 `Not-tested:`。

Pull requests 应包含目的说明、关联 goal/issue、API 或行为变化、实际运行的 validation commands，以及发布证据状态。只有文档或生成报告变更需要 screenshots。

## 安全与配置提示

`configx` 必须保持配置加载显式。不得自动读取 `.env`、`production.yaml` 或 `/home/k8s/secrets/env/*`。不得在 errors、tests、release manifests、examples 或日志中记录原始 secret 值。禁止引入 `x.go`、交易领域术语或基础设施驱动依赖，除非目标文档和边界脚本同步更新并经过审查。
