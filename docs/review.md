# configx 模板应用复盘（template application review）

评审已应用的 baselib template 与 `configx` goal 是否一致时，使用本 checklist。

## 2026-06-01 模板与 foundation 复盘记录（template/foundation review notes）
...
- 边界校验（Boundary validation）必须在 release evidence 被接受前拒绝生成的 `x.go` files、`x.go` dependencies 与 infrastructure driver dependencies。
- 密钥证据（Secret evidence）只能使用 redacted values；不得在 validation logs、examples、manifests 或 documentation output 中调用 `Reveal()`。
