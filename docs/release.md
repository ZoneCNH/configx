...
- loaders、merge precedence、decode、validation 与 sanitization 的 unit tests 通过
- 如果支持 concurrency，loader/result paths with shared readers 的 race tests 通过
- config schema、error envelope 与 release manifest 的 contract checks 通过
...
Required manifest fields 包括 module path、version、commit、tree state、checks、contract hashes、dependencies、tools、artifacts 与 known risks。
