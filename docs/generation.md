...
`scripts/render_template.sh` 用于把 `configx` 渲染为具体基础库，例如 `foundationx`。脚本负责同步替换 module name、module path、package name、`pkg/` 目录名、imports、文档占位符和脚本中的模板名称。
...
- `github.com/bytechainx/configx` 和 `github.com/bytechainx/configx` 替换为 `--module-path`。
