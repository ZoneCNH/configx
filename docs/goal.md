...
> 分层定位：L1 运行时基础能力层  
> 上游依赖：`github.com/bytechainx/foundationx`  
...
```text
1. 当前目标是创建或完善独立 Go module：github.com/bytechainx/configx
...
```text
1. target 必须是 pointer to struct
...
```text
AC-REQ-CONFIGX-001-001: go.mod module 为 github.com/bytechainx/configx
...
```text
AC-REQ-CONFIGX-009-001: Decode 要求 target 为 pointer to struct
...
mkdir -p configx
cd configx
go mod init github.com/bytechainx/configx
...
```bash
go get github.com/bytechainx/foundationx
```
...
|---|---|---|---|---|---|---|
| REQ-CONFIGX-001 | AC-001-* | Module Design | TASK-001 | go test ./... | EVID-001 | TODO |
| REQ-CONFIGX-002 | AC-002-* | Boundary | TASK-016 | boundary gate | EVID-016 | TODO |
...
configx 不持有全局配置
configx 不泄露 secret
configx v0.1.0 release evidence 完整
...
2. 接入 foundationx
3. 编写 explicit source loading ADR
...
你必须创建或完善 github.com/bytechainx/configx。
...
9. configx 不允许在日志、错误、Evidence 中输出 secret 原值。
...
```text
v0.1.0 = 显式配置加载 + 多来源合并 + struct decode + secret 脱敏 + Harness + Release Evidence
```
